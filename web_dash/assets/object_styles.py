...
def draw_objects(fig, df_o: pd.DataFrame, df_c: pd.DataFrame, tf_minutes: int, variant: str = "zones"):
    """
    Draws levels/zones using object 'left' aligned to candle global_x.
    df_c must have '_ts_plot' and 'global_x'.
    """
    if df_o.empty:
        return
    
    styles = load_object_styles(variant)
    gx_ts = _gx_lookup(df_c)
    x_end = df_c["_ts_plot"].iloc[-1] + pd.Timedelta(minutes=tf_minutes)
    ...
    for _, obj in df_o.iterrows():
        ...
        if pd.notna(obj.get("y")):
            y = float(obj["y"])
            fig.add_shape(
                type="line", x0=start, x1=x_end, y0=y, y1=y,
                xref="x", yref="y",
                line=dict(color=st.line, width=st.level_width),
                layer="above",
            )
            fig.add_scatter(
                x=[start], y=[y], mode="markers",
                marker=dict(size=6, color=st.line), showlegend=False,
                hoverinfo="skip",
            )
        elif pd.notna(obj.get("top")) and pd.notna(obj.get("bottom")):
            y0, y1 = sorted([float(obj["top"]), float(obj["bottom"])])
            fig.add_shape(
                type="rect", x0=start, x1=x_end, y0=y0, y1=y1,
                xref="x", yref="y",
                line=dict(width=0),
                fillcolor=st.fill, opacity=st.zone_opacity,
                layer="below",
            )
