...
from data_acquisition import add_markers, is_ema_broke, get_current_candle_index, get_latest_ema_values, update_order_details, read_config, get_current_price
from order_utils import build_active_order, calculate_bid_percentage
        ...
        while current_order_active:  # Loop to manage an individual order
            ...
            try:
                ...
                if remaining_quantity <= 0:
                    ...
                    print_log(f"(manage_active_order) Profit/Loss: ${profit_loss:.2f}")
...
async def check_stop_loss(current_bid_price, buy_entry_price, position_type, last_check_candle_index, last_checked_ema_index):
    ...
    if isinstance(STOP_LOSS, str): # STOP_LOSS is string
        ...
        if "EMA" in STOP_LOSS:
            ...
            if is_ema_broke(ema_value, read_config('SYMBOL'), read_config('TIMEFRAMES')[0], position_type):
        ...
        if current_loss <= STOP_LOSS:
            print_log(f"[STOP LOSS] Hit: {current_loss:.2f}% <= {STOP_LOSS}%")
    ...
    # e.g., ["SENTIMENT", 2], Stoploss is whatever sentiment switches to the oposing threshold (2)
        ...
        if isinstance(SL_string, str) and "EMA" in SL_string and isinstance(SL_number, (int, float)):
            ...
            if (current_candle_index != last_check_candle_index 
                ...
                broke = is_ema_broke(ema_value, read_config('SYMBOL'), read_config('TIMEFRAMES')[0], position_type)
                if broke:
